...
template <typename F>
Point NewtonMethod(const Point& X0, bool useGSS, F& f, double eps, int maxIter = 10000)
{
  Point Xold;
  Point Xnew = X0;

  auto fGSS = [&](double lambda) {
    return f(Xold - lambda * Inverse(f.hess(Xold)) * f.df(Xold));
  };
  ...
  do {
    ...
    Xnew = Xold - gamma * Inverse(f.hess(Xold)) * f.df(Xold);
    ++iter;
